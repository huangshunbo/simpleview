package com.android.minlib.simplewidget.tab;
...
public class TabLayout extends HorizontalScrollView {
    ...
    /**
     * TAB宽度的类型,0 充满整个控件宽度,1跟随TITLE的宽度,2跟随ICON的宽度
     * */
    public static final int TAB_WIDTH_FILL = 0;
    public static final int TAB_WIDTH_WITH_TITLE = 1;
    public static final int TAB_WIDTH_WIDTH_ICON = 2;
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTabWidthType = a.getInt(R.styleable.TabLayout_tabIndicatorWidth,TAB_WIDTH_FILL);
        a.recycle();
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private void updateIndicatorPosition() {
            Log.d("hsb","mSelectedPosition "+mSelectedPosition);
            TabView tabView = (TabView) getChildAt(mSelectedPosition);
            int left = 0, right = 0;

            if (tabView != null && tabView.getWidth() > 0) {
                if(mTabWidthType == TAB_WIDTH_FILL){
                    left = tabView.getLeft();
                    right = tabView.getRight();
                }else if(mTabWidthType == TAB_WIDTH_WITH_TITLE){
                    left = tabView.getWidth() * mSelectedPosition + tabView.getTextView().getLeft();
                    right = tabView.getWidth() * mSelectedPosition + tabView.getTextView().getRight();
                }else if(mTabWidthType == TAB_WIDTH_WIDTH_ICON){
                    left = tabView.getWidth() * mSelectedPosition + tabView.getIconView().getLeft();
                    right = tabView.getWidth() * mSelectedPosition + tabView.getIconView().getRight();
                }
                if (mSelectionOffset > 0f && mSelectedPosition < getChildCount() - 1) {
                    ...
                    if(mTabWidthType == TAB_WIDTH_FILL){
                        ...
                        left = (int) (mSelectionOffset * (nextTabView.getWidth()*(mSelectedPosition+1) + nextTabView.getTextView().getLeft()) +
                                (1.0f - mSelectionOffset) * left);
                        right = (int) (mSelectionOffset * (nextTabView.getWidth()*(mSelectedPosition+1) + nextTabView.getTextView().getRight()) +
                                (1.0f - mSelectionOffset) * right);
                    }else if(mTabWidthType == TAB_WIDTH_WIDTH_ICON){
                        left = (int) (mSelectionOffset * (nextTabView.getWidth()*(mSelectedPosition+1) + nextTabView.getIconView().getLeft()) +
                                (1.0f - mSelectionOffset) * left);
                        right = (int) (mSelectionOffset * (nextTabView.getWidth()*(mSelectedPosition+1) + nextTabView.getIconView().getRight()) +
                                (1.0f - mSelectionOffset) * right);
                    }
                }
            } else {
