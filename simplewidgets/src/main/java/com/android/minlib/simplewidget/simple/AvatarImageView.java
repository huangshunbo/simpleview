...
/**
 * @author: huangshunbo
 * @Filename: AvatarImageView
 * @Description: 圆形文字 或 圆形图片 https://github.com/Carbs0126/AvatarImageView
...
public class AvatarImageView extends ImageView {
    ...
    private int mBgColor;//background color when show text
    ...
    protected void onDraw(Canvas canvas) {
        if (mBitmap != null && mType == DEFAULT_TYPE_BITMAP) {
            toDrawBitmap(canvas);
        } else if ( !TextUtils.isEmpty(mText) && mType == DEFAULT_TYPE_TEXT) {
            toDrawText(canvas);
        }
        if((mBitmap != null || !TextUtils.isEmpty(mText)) && mShowBoarder){
            drawBoarder(canvas);
        }
    }
