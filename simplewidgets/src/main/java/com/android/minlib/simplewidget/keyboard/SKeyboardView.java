package com.android.minlib.simplewidget.keyboard;
...
/**
 * @author: huangshunbo
 * @Filename: SKeyboardView
 * @Description: 自定义键盘，按要求绘制键盘背景，图标，文本等
...
//        }
    ...
    private void drawKeyboardNumber(List<Keyboard.Key> keys, Canvas canvas) {
        for (Keyboard.Key key : keys) {
            if (key.codes[0] == -5) {//删除键
                drawKeyBackground(R.drawable.keyboard_img_edit_clear, canvas, key);
            }
        }
    }
    ...
    private void drawKeyBackground(int drawableId, Canvas canvas, Keyboard.Key key) {
        Drawable npd = ContextCompat.getDrawable(context, drawableId);
